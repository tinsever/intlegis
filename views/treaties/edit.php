<div class="md:flex md:items-center md:justify-between mb-8 pb-4 border-b border-gray-100">
    <div class="flex-1 min-w-0">
        <h2 class="text-2xl font-bold leading-7 text-gray-900 sm:text-3xl sm:truncate">
            Vertrag bearbeiten: <span class="text-indigo-600"><?= htmlspecialchars($treaty['stable_id']) ?></span>
        </h2>
        <p class="mt-1 text-sm text-gray-500">Das Aktualisieren der Metadaten oder des Inhalts erstellt eine neue Audit-Version.</p>
    </div>
</div>

<form action="/treaties/<?= $treaty['id'] ?>/update" method="POST" class="space-y-8">
        ...
        <div class="space-y-6">
            <div class="bg-gray-50 p-6 rounded-xl border border-gray-200 shadow-sm space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="stable_id" id="stable_id" value="<?= htmlspecialchars($treaty['stable_id']) ?>" required class="form-input">
                    <p class="mt-1 text-xs text-amber-600 font-medium tracking-tight">Warnung: Das Ändern dieser ID kann externe Referenzen beeinträchtigen.</p>
                </div>
                    ...
                    <div>
                        ...
                        <select name="type" id="type" class="form-input">
                            ...
                            foreach ($types as $val => $label): ?>
                                <option value="<?= $val ?>" <?= $treaty['type'] === $val ? 'selected' : '' ?>><?= $label ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="status" id="status" class="form-input">
                            ...
                            foreach ($statuses as $val => $label): ?>
                                <option value="<?= $val ?>" <?= $treaty['status'] === $val ? 'selected' : '' ?>><?= $label ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                </div>
            ...
            <div class="bg-gray-50 p-6 rounded-xl border border-gray-200 shadow-sm space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" name="signature_date" id="signature_date" value="<?= htmlspecialchars($treaty['signature_date'] ?? '') ?>" class="form-input">
                    </div>
                    <div>
                        <label for="in_force_date" class="block text-sm font-medium text-gray-700">In Kraft seit</label>
                        <input type="date" name="in_force_date" id="in_force_date" value="<?= htmlspecialchars($treaty['in_force_date'] ?? '') ?>" class="form-input">
                    </div>
                </div>

                <div>
                    <label for="duration" class="block text-sm font-medium text-gray-700">Laufzeit / Kündigungsfrist</label>
                    <input type="text" name="duration" id="duration" value="<?= htmlspecialchars($treaty['duration'] ?? '') ?>" class="form-input">
                </div>

                <div>
                    <label for="tags" class="block text-sm font-medium text-gray-700">Thematische Tags (kommagetrennt)</label>
                    <input type="text" name="tags" id="tags" value="<?= htmlspecialchars($tags) ?>" class="form-input">
                </div>
            </div>
        </div>
                ...
                <?php foreach ($countries as $c): ?>
                    <label class="relative flex items-start p-3 rounded-lg border border-gray-100 hover:bg-gray-50 cursor-pointer transition-colors group">
                        <div class="flex items-center h-5">
                            <input type="checkbox" name="parties[]" value="<?= $c['id'] ?>" <?= in_array($c['id'], $currentPartyIds) ? 'checked' : '' ?> class="h-4 w-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500">
                        </div>
    ...
    <div class="bg-white p-6 rounded-xl border border-gray-200 shadow-sm" x-data="{ 
        ...
        addAmendment(type, id, title) {
            ...
            document.getElementById('search-results-treaty-edit').innerHTML = '';
        ...
        <div class="mt-2">
            <?php 
                $content_html = $treaty['content_html'];
                $content_json = $treaty['content_json'];
        ...
        <div class="mt-12 pt-8 border-t border-gray-100">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-4">
                    ...
                    <div class="relative" @click.away="document.getElementById('search-results-treaty-edit').innerHTML = ''">
                        <input type="text" 
                               class="form-input" 
                               placeholder="Titel oder Nummer eingeben..."
                               hx-get="<?= url('/api/search-entities') ?>" 
                               hx-trigger="keyup changed delay:300ms" 
                               hx-target="#search-results-treaty-edit"
                               name="q"
                               x-ref="searchInput"
                               autocomplete="off">
                        <div id="search-results-treaty-edit" class="absolute z-20 w-full mt-1 bg-white border border-gray-200 shadow-xl max-h-60 overflow-y-auto"></div>
                    </div>
                </div>
    ...
    <div class="flex items-center justify-end space-x-4 pt-4 border-t border-gray-100">
        <a href="/treaties/<?= $treaty['id'] ?>" class="btn btn-secondary">Abbrechen</a>
